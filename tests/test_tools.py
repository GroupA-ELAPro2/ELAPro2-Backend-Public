...
class testTools(unittest.TestCase):
    ...
    def setUp(self):
        self.bandtool = BandDescriptorTool("ielts_descriptors_academic.json", "ielts_descriptors_general.json")
        self.criteria_tool = KeyAssessmentCriteriaTool("ielts_assessment_criteria_academic.json", "ielts_assessment_criteria_general.json")
        return super().setUp()
    
    #Test band descriptor tool
    def test_task_desc_tool(self):
        output = self.criteria_tool.get_task_description(1)
        self.assertEqual(output,"Letter writing task with defined context and purpose. Each task sets out the context and purpose of the letter and the functions the candidate should cover.")
        output = self.criteria_tool.get_task_description(2)
        self.assertEqual(output,"Formulate and develop a position in relation to a given prompt in the form of a question or statement. Ideas should be supported by evidence, and examples may be drawn from candidate's own experience.")
        output = self.criteria_tool.get_task_description(3)
        self.assertEqual(output,"Information-transfer task relating to factual content of a diagram, graph, table, chart, map or other visual input")
    ...
    def test_band_criteria(self):
        ...
        self.assertIsInstance(output,str)
        output = self.bandtool.get_all_band_descriptors_by_criteria(4,"Coherence_Agent")
        self.assertIsInstance(output,str)
