from agent_files.image_description_agent import image_evaluation, image_agent, is_image_url
...
class TestImageDesc(unittest.TestCase):
    ...
    def setUp(self):
        self.input_data = {"essay":"The answer to the meaning of life is 42. But I'm no expert on this.","question":"What is the meaning of life?",\
            "track_id":"test track","essay_type":2,"target_band":5,\
                "image_url":"https://engnovatewebsitestorage.blob.core.windows.net/ielts-writing-task-1-images/a0977f9b9c675777"}
        return super().setUp()
    
    #Test for valid URL
    def test_url_exists(self):
        result = is_image_url(self.input_data['image_url'])
        self.assertTrue(result)
        
    def test_invalid_url(self):
        result = is_image_url("email@email.com")
        self.assertFalse(result)
        result = is_image_url("https://www.somefakeurl.xxx")
        self.assertFalse(result)
    ...
    def test_image_evaluation(self):
        ...
        print(description)
        self.assertIsInstance(description,ImageDescriptionOutput,f"Returned: {description}")
        
    def test_not_image(self):
        result = is_image_url('https://www.google.com/')
        self.assertFalse(result)
    ...
    def test_not_image_return_value(self):
        ...
        self.assertEqual(image_evaluation(InputState(**self.input_data)),ImageDescriptionOutput(image_description="Google Homepage"))
    
    def test_no_url_function(self):
        self.input_data['image_url'] = None
        self.assertEqual(image_evaluation(InputState(**self.input_data)),ImageDescriptionOutput(image_description=None))

    def test_no_url_agent(self):
        self.input_data['image_url'] = None
        self.assertEqual(image_agent(InputState(**self.input_data)),{'image_description':None})
    ...
    def test_return_existing_desc(self):
        ...
        self.assertEqual(description,{"image_description":"This is a mock image descritption"})
