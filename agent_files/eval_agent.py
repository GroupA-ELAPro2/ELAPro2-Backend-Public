...
def safe_json_parse(text: str):
    ...
    Returns a Python dict or raises a descriptive error.
    """
...
# Evaluation Function (save to JSON)

def evaluate_model_output_to_json(data: CentralAgentState, base_dir: str = "evaluations"):
    """
    Evaluates another model's feedback against IELTS standards and saves results as a JSON file
    dynamically named with track_id and current timestamp.
    """
    ...
    agent_names = {1:"task_agent",
                   2:"grammar_agent",
                   3:"lexical_agent",
                   4:"coherence_agent"}    
    
    # get the band descriptor for the each criteria depending on the actual score
    
    descritors_task = t.get_target_band_descriptors_by_criteria.func(data.essay_type, agent_names[1], int(data.task_score) ),
    descritors_grammar = t.get_target_band_descriptors_by_criteria.func(data.essay_type, agent_names[2], int(data.grammar_score)),
    descritors_lexical = t.get_target_band_descriptors_by_criteria.func(data.essay_type, agent_names[3], int(data.lexical_score)),
    descritors_coherence = t.get_target_band_descriptors_by_criteria.func(data.essay_type, agent_names[4], int(data.coherence_score))

    # band_output = round(calculate_overall_band([data.task_score,data.grammar_score, data.lexical_score, data.coherence_score]), 0)
    ...
    # get the content part of the llm response

    response_content = safe_json_parse(reponse.content)

    # get the metadata part of the llm response

    response_metadata = reponse.response_metadata

    # keep only the information of interest
    ...
    # transform the output state into a dictionary
...
# Meta-Eval Wrapper (runs in parallel, to generate a side output)

# def meta_eval_wrapper(input: CentralAgentState) -> EvaluationOutputState:
#     result = evaluate_model_output_to_json(input)  # returns EvaluationOutputState
#     return result

# Build SubGraph

subgraph_builder = StateGraph(EvaluationOutputState, input_schema=CentralAgentState, output_schema=EvaluationOutputState)
