...
# Evaluate Grammatical and Rage Accuracy using assessment criteria tool
def grammar_evaluation(input_state: ToolState) -> GrammarOutputState:
    """Evaluate essay grammatical range and accuracy using IELTS criteria.

    Args:
        input_state (ToolState): Tool state containing essay, question, and assessment tools.

    Returns:
        GrammarOutputState: Grammar band score and feedback comments.
    """
    ...
    prompt = IELTS_EVALUATION_PROMPT.format(
        ...
        goal_aditional="Do NOT comment on sentence structure, coherence and cohesion; vocabulary range, accuracy, appropriacy, sophistication; and task achievement.",
        notes="Indicate the type of grammatical error in each example."
...
subgraph_builder = StateGraph(
    ToolState, input_schema=InputState, output_schema=GrammarOutputState
)
...
def grammar_agent(data):
    """Execute the grammar evaluation agent workflow.

    Args:
        data: Input data for grammar assessment.

    Returns:
        Grammar evaluation results from the graph execution.
    """
